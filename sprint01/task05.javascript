5. Write a checkAdult(age) function whose input parameter is the age of the user age. The function checks whether the set age parameter is set correctly, if it is set incorrectly, the corresponding error should be generated and displayed in the console:
...
function checkAdult(age){
    try {
      if(age >=18 && Number.isInteger(age)){console.log('Access allowed'); }
      if(age === "") { throw "Error Please, enter your age";}
      if(age === undefined) { throw "Error Please, enter your age";}
      if(isNaN(age)) { throw "Error Please, enter number";}
      if(age < 0) { throw "Error Please, enter positive number";}
      if(age < 18) { throw "Error Access denied - you are too young!";}
      if(!Number.isInteger(age)) {throw "Error Please, enter Integer number";}
    }
  catch(error) {
   console.log(error)
     } 
