...
//розвязок
function longestLogin(loginList){
  return loginList.reduce(function(result, login) {
    return result.length > login.length ? result : login;
  }, "");
}

//розвязок
function longestLogin(loginList){
  return loginList.reduce((result, login) => login.length >= result.length ? login : result, '');
}
